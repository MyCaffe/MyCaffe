//=============================================================================
//  Copyright (c) 2016-2023, SignalPOP.  All rights reserved.
//
//	--CAFFE--
...
template <typename T>
inline __device__ T sigmoid1(const T x);

template <>
inline __device__ double sigmoid1(const double x)
{
	return 1.0 / (1.0 + exp(-x));
}

template <>
inline __device__ float sigmoid1(const float x)
{
	return 1.0f / (1.0f + expf(-x));
}

template <typename T>
inline __device__ T log1(const T x);

template <>
inline __device__ double log1(const double x)
{
	return log(x);
}

template <>
inline __device__ float log1(const float x)
{
	return logf(x);
}

template <typename T>
inline __device__ T tanh(const T x)
{
	return T(2) * sigmoid1(T(2) * x) - T(1);
}
...
long Math<double>::nrm2(int n, long hA, int nAOff, double* pdfResult)
{
	...
	if (lErr = cublasDnrm2(cublas, n, a, 1, pdfResult))
		return lErr | ERROR_CUBLAS_OFFSET;
...
long Math<float>::nrm2(int n, long hA, int nAOff, float* pfResult)
{
	...
	if (lErr = cublasSnrm2(cublas, n, a, 1, pfResult))
		return lErr | ERROR_CUBLAS_OFFSET;
...
long Math<double>::ger(int m, int n, double fAlpha, long hA, long hB, long hC, int nAoff, int nBoff, int nCoff)
{
	...
	if (lErr = cublasDger(cublas, n, m, &fAlpha, a, 1, b, 1, c, m))
		return lErr | ERROR_CUBLAS_OFFSET;
...
long Math<float>::ger(int m, int n, float fAlpha, long hA, long hB, long hC, int nAoff, int nBoff, int nCoff)
{
	...
	if (lErr = cublasSger(cublas, n, m, &fAlpha, a, 1, b, 1, c, m))
		return lErr | ERROR_CUBLAS_OFFSET;
...
long Math<double>::gemm(bool bTransA, bool bTransB, int m, int n, int k, double fAlpha, double* a, double* b, double fBeta, double* c, cudaStream_t stream)
{
	...
	if (lErr = cublasDgemm(cublas, cuTransB, cuTransA, n, m, k, &fAlpha, b, ldb, a, lda, &fBeta, c, n))
		return lErr | ERROR_CUBLAS_OFFSET;
...
long Math<float>::gemm(bool bTransA, bool bTransB, int m, int n, int k, float fAlpha, float* a, float *b, float fBeta, float* c, cudaStream_t stream)
{
	...
	if (lErr = cublasSgemm(cublas, cuTransB, cuTransA, n, m, k, &fAlpha, b, ldb, a, lda, &fBeta, c, n))
		return lErr | ERROR_CUBLAS_OFFSET;
...
long Math<double>::gemm(bool bTransA, bool bTransB, int m, int n, int k, double dfAlpha, __half* a, __half *b, double dfBeta, __half* c, cudaStream_t stream)
{
	...
	if (lErr = cublasGemmEx(cublas, cuTransB, cuTransA, n, m, k, &fAlpha, b, CUDA_R_16F, ldb, a, CUDA_R_16F, lda, &fBeta, c, CUDA_R_16F, n, CUDA_R_32F, CUBLAS_GEMM_DEFAULT))
		return lErr | ERROR_CUBLAS_OFFSET;
...
long Math<double>::gemm2(bool bTransA, bool bTransB, int m, int n, int k, double fAlpha, long hA, long hB, double fBeta, long hC, int lda, int ldb, int ldc)
{
	...
	if (pA->IsHalf() && pB->IsHalf() && pC->IsHalf())
	{
		...
		if (lErr = cublasGemmEx(cublas, cuTransA, cuTransB, m, n, k, &fAlpha, a, CUDA_R_16F, lda, b, CUDA_R_16F, ldb, &fBeta, c, CUDA_R_16F, ldc, CUDA_R_32F, CUBLAS_GEMM_DEFAULT))
			return lErr | ERROR_CUBLAS_OFFSET;
	}
	else
	{
		...
		if (lErr = cublasDgemm(cublas, cuTransA, cuTransB, m, n, k, &fAlpha, a, lda, b, ldb, &fBeta, c, ldc))
			return lErr | ERROR_CUBLAS_OFFSET;
	}
...
long Math<float>::gemm2(bool bTransA, bool bTransB, int m, int n, int k, float fAlpha, long hA, long hB, float fBeta, long hC, int lda, int ldb, int ldc)
{
	...
	else
	{
		...
		if (lErr = cublasSgemm(cublas, cuTransA, cuTransB, m, n, k, &fAlpha, a, lda, b, ldb, &fBeta, c, ldc))
			return lErr | ERROR_CUBLAS_OFFSET;
	}
...
long Math<double>::geam(bool bTransA, bool bTransB, int m, int n, double fAlpha, double* a, double* b, double fBeta, double* c)
{
	...
	if (lErr = cublasDgeam(cublas, cuTransA, cuTransB, m, n, &fAlpha, a, lda, &fBeta, b, ldb, c, m))
		return lErr | ERROR_CUBLAS_OFFSET;
...
long Math<float>::geam(bool bTransA, bool bTransB, int m, int n, float fAlpha, float* a, float* b, float fBeta, float* c)
{
	...
	if (lErr = cublasSgeam(cublas, cuTransA, cuTransB, m, n, &fAlpha, a, lda, &fBeta, b, ldb, c, m))
		return lErr | ERROR_CUBLAS_OFFSET;
...
long Math<double>::gemv(bool bTransA, int m, int n, double fAlpha, double* a, double* x, double fBeta, double* y)
{
	...
	if (lErr = cublasDgemv(cublas, cuTransA, n, m, &fAlpha, a, n, x, 1, &fBeta, y, 1))
		return lErr | ERROR_CUBLAS_OFFSET;
...
long Math<float>::gemv(bool bTransA, int m, int n, float fAlpha, float* a, float* x, float fBeta, float* y)
{
	...
	if (lErr = cublasSgemv(cublas, cuTransA, n, m, &fAlpha, a, n, x, 1, &fBeta, y, 1))
		return lErr | ERROR_CUBLAS_OFFSET;
...
long Math<double>::axpy(int n, double fAlpha, long hX, long hY, int nXOff, int nYOff)
{
	...
	if (pX->IsHalf() && pY->IsHalf())
	{
		...
		if (lErr = cublasAxpyEx(cublas, n, &fAlpha1, CUDA_R_32F, x, CUDA_R_16F, 1, y, CUDA_R_16F, 1, CUDA_R_32F))
			return lErr | ERROR_CUBLAS_OFFSET;
	}
	else
	{
		...
		if (lErr = cublasDaxpy(cublas, n, &fAlpha, x, 1, y, 1))
			return lErr | ERROR_CUBLAS_OFFSET;
	}
...
long Math<float>::axpy(int n, float fAlpha, long hX, long hY, int nXOff, int nYOff)
{
	...
	if (pX->IsHalf() && pY->IsHalf())
	{
		...
		if (lErr = cublasAxpyEx(cublas, n, &fAlpha, CUDA_R_32F, x, CUDA_R_16F, 1, y, CUDA_R_16F, 1, CUDA_R_32F))
			return lErr | ERROR_CUBLAS_OFFSET;
	}
	else
	{
		...
		if (lErr = cublasSaxpy(cublas, n, &fAlpha, x, 1, y, 1))
			return lErr | ERROR_CUBLAS_OFFSET;
	}
...
long Math<double>::axpby(int n, double fAlpha, long hX, double fBeta, long hY)
{
	...
	if (pY->IsHalf())
	{
		...
		if (lErr = cublasScalEx(cublas, n, &fBeta1, CUDA_R_32F, x, CUDA_R_16F, 1, CUDA_R_32F))
			return lErr | ERROR_CUBLAS_OFFSET;
	}
	else
	{
		if (lErr = cublasDscal(cublas, n, &fBeta, (double*)pY->Data(), 1))
			return lErr | ERROR_CUBLAS_OFFSET;
	}
...
long Math<float>::axpby(int n, float fAlpha, long hX, float fBeta, long hY)
{
	...
	if (pY->IsHalf())
	{
		...
		if (lErr = cublasScalEx(cublas, n, &fBeta, CUDA_R_32F, x, CUDA_R_16F, 1, CUDA_R_32F))
			return lErr | ERROR_CUBLAS_OFFSET;
	}
	else
	{
		if (lErr = cublasSscal(cublas, n, &fBeta, (float*)pY->Data(), 1))
			return lErr | ERROR_CUBLAS_OFFSET;
	}
...
long Math<double>::scal(int n, double fAlpha, long hX, int nXOff, long hStream)
{
	...
	if (hStream != 0)
	{
		...
		if (lErr = cublasGetStream(cublas, &initial_stream))
			return lErr | ERROR_CUBLAS_OFFSET;

		if (lErr = cublasSetStream(cublas, stream))
			return lErr | ERROR_CUBLAS_OFFSET;
	}
	...
	if (pX->IsHalf())
	{
		...
		if (lErr = cublasScalEx(cublas, n, &fAlpha1, CUDA_R_32F, x, CUDA_R_16F, 1, CUDA_R_32F))
			return lErr | ERROR_CUBLAS_OFFSET;
	}
	else
	{
		...
		if (lErr = cublasDscal(cublas, n, &fAlpha, x, 1))
			return lErr | ERROR_CUBLAS_OFFSET;
	}
	...
	if (initial_stream != NULL)
		cublasSetStream(cublas, initial_stream);
...
long Math<float>::scal(int n, float fAlpha, long hX, int nXOff, long hStream)
{
	...
	if (hStream != 0)
	{
		...
		if (lErr = cublasSetStream(cublas, stream1))
			return lErr | ERROR_CUBLAS_OFFSET;
	}
	...
	if (pX->IsHalf())
	{
		...
		if (lErr = cublasScalEx(cublas, n, &fAlpha, CUDA_R_32F, x, CUDA_R_16F, 1, CUDA_R_32F))
			return lErr | ERROR_CUBLAS_OFFSET;
	}
	else
	{
		...
		if (lErr = cublasSscal(cublas, n, &fAlpha, x, 1))
			return lErr | ERROR_CUBLAS_OFFSET;
	}
...
long Math<double>::dot(int n, long hX, long hY, double* pOut, int nXOff, int nYOff)
{
	...
	if (pX->IsHalf() && pY->IsHalf())
	{
		...
		if (lErr = cublasDotEx(cublas, n, x, CUDA_R_16F, 1, y, CUDA_R_16F, 1, &result, CUDA_R_16F, CUDA_R_32F))
			return lErr | ERROR_CUBLAS_OFFSET;
	...
	else
	{
		...
		if (lErr = cublasDdot(cublas, n, x, 1, y, 1, pOut))
			return lErr | ERROR_CUBLAS_OFFSET;
	}
...
long Math<float>::dot(int n, long hX, long hY, float* pOut, int nXOff, int nYOff)
{
	...
	else
	{
		...
		if (lErr = cublasSdot(cublas, n, x, 1, y, 1, (float*)pOut))
			return lErr | ERROR_CUBLAS_OFFSET;
	}
...
long Math<double>::asum(int n, long hX, double* pOut, int nXOff)
{
	...
	if (lErr = cublasDasum(cublas, n, x, 1, (double*)pOut))
		return lErr | ERROR_CUBLAS_OFFSET;
...
long Math<float>::asum(int n, long hX, float* pOut, int nXOff)
{
	...
	if (lErr = cublasSasum(cublas, n, x, 1, (float*)pOut))
		return lErr | ERROR_CUBLAS_OFFSET;
...
long Math<double>::asum(int n, double* x, double* pOut)
{
	...
	if (lErr = cublasDasum(cublas, n, x, 1, pOut))
		return lErr | ERROR_CUBLAS_OFFSET;
...
long Math<float>::asum(int n, float* x, float* pOut)
{
	...
	if (lErr = cublasSasum(cublas, n, x, 1, pOut))
		return lErr | ERROR_CUBLAS_OFFSET;
...
long Math<double>::scale(int n, double fAlpha, long hX, long hY, int nXOff, int nYOff)
{
	...
	if (lErr = cublasDcopy(cublas, n, x, 1, y, 1))
		return lErr | ERROR_CUBLAS_OFFSET;

	if (lErr = cudaStreamSynchronize(stream))
		return lErr;

	if (lErr = cublasDscal(cublas, n, &fAlpha, y, 1))
		return lErr | ERROR_CUBLAS_OFFSET;
...
long Math<float>::scale(int n, float fAlpha, long hX, long hY, int nXOff, int nYOff)
{
	...
	if (lErr = cublasScopy(cublas, n, x, 1, y, 1))
		return lErr | ERROR_CUBLAS_OFFSET;

	if (lErr = cudaStreamSynchronize(stream))
		return lErr;

	if (lErr = cublasSscal(cublas, n, &fAlpha, y, 1))
		return lErr | ERROR_CUBLAS_OFFSET;
...
template <>
long Math<double>::scale(int n, double fAlpha, const double* x, double* y)
{
	cublasHandle_t cublas;
	if ((cublas = GetCublasHandle()) == NULL)
		return ERROR_CUBLAS_NULL;
...
template <>
long Math<float>::scale(int n, float fAlpha, const float* x, float* y)
{
	cublasHandle_t cublas;
	if ((cublas = GetCublasHandle()) == NULL)
		return ERROR_CUBLAS_NULL;
...
template <typename T>
__global__ void invert_kernel(const int n, T* x, T* y)
{
	for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < n && i >= 0; i += blockDim.x * gridDim.x)
	{
		y[i] = (x[i] == 0) ? 0 : (T)1 / x[i];
...
template <typename T>
__global__ void channel_min_kernel_acrosschannels(const int num, const int channels, const int spatial_dim, const T* x, T* y)
{
	for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < num * spatial_dim && i >= 0; i += blockDim.x * gridDim.x)
	{
		...
		for (int c = 0; c < channels; c++)
		{
			val = min(x[(offset + c) * spatial_dim + s], val);
		}
...
template <typename T>
__global__ void channel_min_kernel_withinchannel(const int num, const int channels, const int spatial_dim, const T* x, T* y)
{
	for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < num * spatial_dim && i>=0; i += blockDim.x * gridDim.x)
	{
...
template <typename T>
__global__ void channel_min_idx_kernel_acrosschannels(const int num, const int channels, const int spatial_dim, const T* x, T* y)
{
	for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < num * spatial_dim && i >= 0; i += blockDim.x * gridDim.x)
	{
		const int n = i / spatial_dim;
		...
		for (int c = 0; c < channels; c++)
		{
			const T val1 = x[(offset + c) * spatial_dim + s];
			if (val1 < val)
			{
				val = val1;
				nIdx = c;
...
template <typename T>
__global__ void channel_min_idx_kernel_withinchannel(const int num, const int channels, const int spatial_dim, const T* x, T* y)
{
	for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < num * spatial_dim && i >= 0; i += blockDim.x * gridDim.x)
	{
		const T* x1 = x + (i * spatial_dim);
...
template <typename T>
long Math<T>::channel_min(int n, int nOutNum, int nChannels, int nInNum, long hX, long hY, bool bReturnIdx, bool bAcrossChannels)
{
	LONG lErr;
	MemoryItem* pX;
	MemoryItem* pY;
	...
	{
		if (bAcrossChannels)
			channel_min_idx_kernel_acrosschannels<T> << <CAFFE_GET_BLOCKS(n), CAFFE_CUDA_NUM_THREADS >> > (nOutNum, nChannels, nInNum, (T*)pX->Data(), (T*)pY->Data());
		else
			channel_min_idx_kernel_withinchannel<T> << <CAFFE_GET_BLOCKS(n), CAFFE_CUDA_NUM_THREADS >> > (nOutNum, nChannels, nInNum, (T*)pX->Data(), (T*)pY->Data());
	}
	else
	{
		if (bAcrossChannels)
			channel_min_kernel_acrosschannels<T> << <CAFFE_GET_BLOCKS(n), CAFFE_CUDA_NUM_THREADS >> > (nOutNum, nChannels, nInNum, (T*)pX->Data(), (T*)pY->Data());
		else
			channel_min_kernel_withinchannel<T> << <CAFFE_GET_BLOCKS(n), CAFFE_CUDA_NUM_THREADS >> > (nOutNum, nChannels, nInNum, (T*)pX->Data(), (T*)pY->Data());
	}

	return cudaStreamSynchronize(0);
}

template long Math<double>::channel_min(int n, int nOutNum, int nChannels, int nInNum, long hX, long hY, bool bReturnIdx, bool bAcrossChannels);
template long Math<float>::channel_min(int n, int nOutNum, int nChannels, int nInNum, long hX, long hY, bool bReturnIdx, bool bAcrossChannels);

template <typename T>
__global__ void channel_max_kernel_acrosschannels(const int num, const int channels, const int spatial_dim, const T* x, T* y)
{
	for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < num * spatial_dim && i >= 0; i += blockDim.x * gridDim.x)
	{
		const int n = i / spatial_dim;
		...
		for (int c = 0; c < channels; c++)
		{
			val = max(x[(offset + c) * spatial_dim + s], val);
		}
...
template <typename T>
__global__ void channel_max_kernel_withinchannel(const int num, const int channels, const int spatial_dim, const T* x, T* y)
{
	for (int i=blockIdx.x * blockDim.x + threadIdx.x; i<num * spatial_dim && i>=0; i += blockDim.x * gridDim.x)
	{
		const T* x1 = x + (i * spatial_dim);
...
template <typename T>
__global__ void channel_max_idx_kernel_acrosschannels(const int num, const int channels, const int spatial_dim, const T* x, T* y)
{
	for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < num * spatial_dim && i >= 0; i += blockDim.x * gridDim.x)
	{
		const int n = i / spatial_dim;
...
template <typename T>
__global__ void channel_max_idx_kernel_withinchannel(const int num, const int channels, const int spatial_dim, const T* x, T* y)
{
	for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < num * spatial_dim && i >= 0; i += blockDim.x * gridDim.x)
	{
		const T* x1 = x + (i * spatial_dim);
...
template <typename T> 
long Math<T>::channel_max(int n, int nOutNum, int nChannels, int nInNum, long hX, long hY, bool bReturnIdx, bool bAcrossChannels)
{
	LONG lErr;
	MemoryItem* pX;
	MemoryItem* pY;
	...
	{
		if (bAcrossChannels)
			channel_max_idx_kernel_acrosschannels<T> << <CAFFE_GET_BLOCKS(n), CAFFE_CUDA_NUM_THREADS >> > (nOutNum, nChannels, nInNum, (T*)pX->Data(), (T*)pY->Data());
		else
			channel_max_idx_kernel_withinchannel<T> << <CAFFE_GET_BLOCKS(n), CAFFE_CUDA_NUM_THREADS >> > (nOutNum, nChannels, nInNum, (T*)pX->Data(), (T*)pY->Data());
	}
	else
	{
		if (bAcrossChannels)
			channel_max_kernel_acrosschannels<T> << <CAFFE_GET_BLOCKS(n), CAFFE_CUDA_NUM_THREADS >> > (nOutNum, nChannels, nInNum, (T*)pX->Data(), (T*)pY->Data());
		else
			channel_max_kernel_withinchannel<T> << <CAFFE_GET_BLOCKS(n), CAFFE_CUDA_NUM_THREADS >> > (nOutNum, nChannels, nInNum, (T*)pX->Data(), (T*)pY->Data());
	}

	return cudaStreamSynchronize(0);
}

template long Math<double>::channel_max(int n, int nOutNum, int nChannels, int nInNum, long hX, long hY, bool bReturnIdx, bool bAcrossChannels);
template long Math<float>::channel_max(int n, int nOutNum, int nChannels, int nInNum, long hX, long hY, bool bReturnIdx, bool bAcrossChannels);


template <typename T>
__global__ void channel_mean_kernel(const int num, const int channels, const int spatial_dim, const T* x, T* y, int nXOff)
{
...
template <typename T>
long Math<T>::channel_mean(int n, int nOutNum, int nChannels, int nInNum, long hX, long hY, int nXOff)
{
	LONG lErr;
	MemoryItem* pX;
	MemoryItem* pY;
	...
	channel_mean_kernel<T> << <CAFFE_GET_BLOCKS(n), CAFFE_CUDA_NUM_THREADS >> > (nOutNum, nChannels, nInNum, (T*)pX->Data(), (T*)pY->Data(), nXOff);

	return cudaStreamSynchronize(0);
}

template long Math<double>::channel_mean(int n, int nOutNum, int nChannels, int nInNum, long hX, long hY, int nXOff);
...
long Math<T>::channel_stdev(int n, int nOutNum, int nChannels, int nInNum, long hX, long hY, long hZ, T fEps, bool bUnbiased)
{
	...
	if (lErr = m_pMemCol->GetData(hZ, &pZ))
		return lErr;

	channel_mean_kernel<T> << <CAFFE_GET_BLOCKS(n), CAFFE_CUDA_NUM_THREADS >> > (nOutNum, nChannels, nInNum, (T*)pX->Data(), (T*)pZ->Data(), 0);
...
template <typename T>
__global__ void channel_sum_kernel_acrosschannels_original(const int num, const int channels, const int spatial_dim, const T* x, T* y)
{
	for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < num * spatial_dim && i >= 0; i += blockDim.x * gridDim.x)
	{
		const int n = i / spatial_dim;
...
long Math<T>::channel_sum(int n, int nOutNum, int nChannels, int nInNum, long hX, long hY)
{
	...
	channel_sum_kernel_acrosschannels_original<T> << <CAFFE_GET_BLOCKS(n), CAFFE_CUDA_NUM_THREADS >> > (nOutNum, nChannels, nInNum, (T*)pX->Data(), (T*)pY->Data());
...
long Math<T>::channel_copy(int n, int nOutNum, int nChannels, int nBlocks, int nInNum, int nOffset, long hX, long hY, int nDir)
{
	...
	if (nDir == 0) // X -> Y
		channel_copy_kernel_fwd<T> << <CAFFE_GET_BLOCKS(n), CAFFE_CUDA_NUM_THREADS >> > (n, nOutNum, nChannels, nBlocks, nInNum, nOffset, x, y);
	else // Y -> X
		channel_copy_kernel_bwd<T> << <CAFFE_GET_BLOCKS(n), CAFFE_CUDA_NUM_THREADS >> > (n, nOutNum, nChannels, nBlocks, nInNum, nOffset, x, y);
...
__global__ void channel_duplicate_kernel(const int nYCount, const int num, const int channels, const int dup_count, const T* x, T* y)
{
	...
	for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < nYCount && i >= 0; i += blockDim.x * gridDim.x)
	{
		const int nSrcIdx = ((i / nNSize) * channels) + (i % channels);
		y[i] = x[nSrcIdx];
	}
}

template <typename T>
long Math<T>::channel_duplicate(int n, int nOutNum, int nChannels, int nInNum, long hX, long hY)
{
	LONG lErr;
	MemoryItem* pX;
	MemoryItem* pY;
	...
	channel_duplicate_kernel<T> << <CAFFE_GET_BLOCKS(n), CAFFE_CUDA_NUM_THREADS >> > (n, nOutNum, nChannels, nInNum, x, y);
...
// percentile kernel - expects x to be sorted along num axis.
...
template <>
long Math<float>::transpose(int n, int m, float* src, float* dst)
{
	LONG lErr;
	float fAlpha = 1.0f;
	float fBeta = 0.0f;
	...
	if (lErr = cublasSgeam(cublas, CUBLAS_OP_T, CUBLAS_OP_N, n, m, &fAlpha, src, m, &fBeta, src, n, dst, n))
		return lErr | ERROR_CUBLAS_OFFSET;
...
template <>
long Math<double>::transpose(int n, int m, double* src, double* dst)
{
	LONG lErr;
	double fAlpha = 1.0f;
	double fBeta = 0.0f;
	...
	if (lErr = cublasDgeam(cublas, CUBLAS_OP_T, CUBLAS_OP_N, n, m, &fAlpha, src, m, &fBeta, src, n, dst, n))
		return lErr | ERROR_CUBLAS_OFFSET;
...
template <typename T>
long Math<T>::channel_percentile(int nCount, int nOuterNum, int nChannels, int nInnerNum, long hX, long hY, T fPercentile)
{
	LONG lErr;
	MemoryItem* pX;
	MemoryItem* pY;
	...
	if (lErr = cudaMalloc(&w1, sz))
		return lErr;
	if (lErr = cudaMalloc(&w2, sz))
	{
		cudaFree(w1);
		return lErr;
	}

	// Transpose to nItems x nOuterNum
	if (lErr = transpose(nOuterNum, nItems, x, w1))
	{
		cudaFree(w1);
		cudaFree(w2);
		return lErr;
	...
	{
		thrust::device_ptr<T> d_ptr = thrust::device_pointer_cast(w1 + nOuterNum * i);
		thrust::sort(d_ptr, d_ptr + nOuterNum);
	}

	// Transpose back to original shape.
	if (lErr = transpose(nItems, nOuterNum, w1, w2))
	{
		cudaFree(w1);
		cudaFree(w2);
		return lErr;
	...
	channel_percentile_kernel<T> <<<CAFFE_GET_BLOCKS(nItems), CAFFE_CUDA_NUM_THREADS >> > (nOuterNum, nItems, w2, y, fPercentile);
...
{
	const int nCountF1 = nN1 * nC * nSD1;
	const int nCountF2 = nN2 * nC * nSD2;
	const int nCount1 = nC * nSD1;
	const int nCount2 = nC * nSD2;

	for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < nYCount && i >= 0; i += blockDim.x * gridDim.x)
	{
		const int n1 = i / nCountF1;
		const int nIdx1 = (n1 % nN1) * nCount1 + (i / nN1) % nCount1;
		const int n2 = i / nCountF2;
		const int nIdx2 = (n2 % nN2) * nCount2 + (i / nN2) % nCount2;
...
long Math<double>::rng_setseed(long lSeed)
{
	...
	if (lErr = curandSetPseudoRandomGeneratorSeed(curand, lSeed))
		return lErr;

	return curandSetGeneratorOffset(curand, 0);
}
...
long Math<double>::rng_uniform(int n, double fMin, double fMax, double* pMem)
{
	...
	if (fRange != 1.0)
	{
		if (lErr = cublasDscal(cublas, n, &fRange, pMem, 1))
			return lErr | ERROR_CUBLAS_OFFSET;
	}
...
long Math<float>::rng_uniform(int n, float fMin, float fMax, float* pMem)
{
	...
	if (lErr = curandGenerateUniform(curand, pMem, n))
		return lErr;

	float fRange = fMax - fMin;
	if (fRange != 1.0)
	{
		if (lErr = cublasSscal(cublas, n, &fRange, pMem, 1))
			return lErr | ERROR_CUBLAS_OFFSET;
	}
...
long Math<double>::rng_gaussian(int n, double fMu, double fSigma, double* pMem, size_t sz)
{
	...
	return curandGenerateNormalDouble(curand, pMem, n, fMu, fSigma);
}
...
long Math<float>::rng_gaussian(int n, float fMu, float fSigma, float* pMem, size_t sz)
{
	...
	return curandGenerateNormal(curand, pMem, n, fMu, fSigma);
}
...
template<typename T>
__global__ void cuda_bce_with_logits_loss_fwd_kernel_pw_w(const int n, const int nN, const int nC, const T* x, const T* y, const T* pw, const T* w, T* work)
{
	for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < n && i >= 0; i += blockDim.x * gridDim.x)
	{
		const int pw_idx = i / nN; // One weight for each item in channel x spatial dimension.
		const int w_idx = i / nC;  // One weight per batch.
		const T pw_val = pw[pw_idx];
		const T w_val = w[w_idx];

		const T input_sigmoid = sigmoid1(x[i]);
		const T log_input_sigmoid = log1(input_sigmoid);
		const T log_1_minus_input_sigmoid = log1((T)1.0 - input_sigmoid);
		T loss = -(pw_val * y[i]) * log_input_sigmoid - ((T)1.0 - y[i]) * log_1_minus_input_sigmoid;
		loss = loss * w_val;
...
template<typename T>
__global__ void cuda_bce_with_logits_loss_fwd_kernel_pw(const int n, int nN, int nC, const T* x, const T* y, const T* pw, T* work)
{
	for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < n && i >= 0; i += blockDim.x * gridDim.x)
	{
		const int pw_idx = i / nN; // One weight for each item in channel x spatial dimension.
...
template<typename T>
__global__ void cuda_bce_with_logits_loss_fwd_kernel_w(const int n, int nN, int nC, const T* x, const T* y, const T* w, T* work)
{
	for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < n && i >= 0; i += blockDim.x * gridDim.x)
	{
		const int w_idx = i / nC;  // One weight per batch.
		...
		const T log_input_sigmoid = log1(input_sigmoid);
		const T log_1_minus_input_sigmoid = log1((T)1.0 - input_sigmoid);
		T loss = -(y[i]) * log_input_sigmoid - ((T)1.0 - y[i]) * log_1_minus_input_sigmoid;
		loss = loss * w_val;
...
template<typename T>
__global__ void cuda_bce_with_logits_loss_fwd_kernel(const int n, const T* x, const T* y, T* work)
{
	for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < n && i >= 0; i += blockDim.x * gridDim.x)
	{
		const T input_sigmoid = sigmoid1(x[i]);
...
template <class T>
long Math<T>::cuda_bce_with_logits_loss_fwd(int n, int nN, long hX, long hY, long hW, long hPosW, long hLossData)
{
	LONG lErr;
	MemoryItem* pX;
	MemoryItem* pY;
	...
	if (hPosW != 0 && hW != 0)
	{
		...
		cuda_bce_with_logits_loss_fwd_kernel_pw_w<<<CAFFE_GET_BLOCKS(n), CAFFE_CUDA_NUM_THREADS>>>(n, nN, nC, x, y, pw, w, loss);
	}
	else if (hPosW != 0)
	{
		if (lErr = m_pMemCol->GetData(hPosW, &pPw))
		...
		cuda_bce_with_logits_loss_fwd_kernel_pw << <CAFFE_GET_BLOCKS(n), CAFFE_CUDA_NUM_THREADS >> > (n, nN, nC, x, y, pw, loss);
	}
	else if (hW != 0)
	{
		if (lErr = m_pMemCol->GetData(hW, &pW))
		...
		cuda_bce_with_logits_loss_fwd_kernel_w << <CAFFE_GET_BLOCKS(n), CAFFE_CUDA_NUM_THREADS >> > (n, nN, nC, x, y, w, loss);
	}
	else
	{
...
template long Math<double>::cuda_bce_with_logits_loss_fwd(int nCount, int nN, long hX, long hY, long hW, long hPosW, long hLossData);
template long Math<float>::cuda_bce_with_logits_loss_fwd(int nCount, int nN, long hX, long hY, long hW, long hPosW, long hLossData);

template<typename T>
__global__ void cuda_bce_with_logits_loss_bwd_kernel_pw_w(const int n, const int nN, const int nC, const T* x, const T* y, const T* pw, const T* w, T* bottom_diff)
{
	for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < n && i >= 0; i += blockDim.x * gridDim.x)
	{
		const T input_sigmoid = sigmoid1(x[i]);
		...
		const T grad = -(pw_val * y[i]) * (1 - input_sigmoid) + (1 - y[i]) * input_sigmoid;
		const T grad_pw = (pw_val * y[i] + (1 - y[i]));		

		bottom_diff[i] = grad * grad_pw * w_val;
	}
}

template<typename T>
__global__ void cuda_bce_with_logits_loss_bwd_kernel_pw(const int n, const int nN, const int nC, const T* x, const T* y, const T* pw, T* bottom_diff)
{
	for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < n && i >= 0; i += blockDim.x * gridDim.x)
	{
		const T input_sigmoid = sigmoid1(x[i]);
...
template<typename T>
__global__ void cuda_bce_with_logits_loss_bwd_kernel_w(const int n, const int nN, const int nC, const T* x, const T* y, const T* w, T* bottom_diff)
{
	for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < n && i >= 0; i += blockDim.x * gridDim.x)
	{
		const T input_sigmoid = sigmoid1(x[i]);
...
long Math<T>::cuda_bce_with_logits_loss_bwd(int n, int nN, long hX, long hY, long hW, long hPosW, bool bMeanReduction, long hBtmDiff)
{
	...
	if (lErr = m_pMemCol->GetData(hBtmDiff, &pBtmDiff))
		return lErr;

	T* x = (T*)pX->Data();
	T* y = (T*)pY->Data();
	T* btm_diff = (T*)pBtmDiff->Data();
	...
	if (hPosW != 0 && hW != 0)
	{
		...
		cuda_bce_with_logits_loss_bwd_kernel_pw_w << <CAFFE_GET_BLOCKS(n), CAFFE_CUDA_NUM_THREADS >> > (n, nN, nC, x, y, pw, w, btm_diff);
	}
	else if (hPosW != 0)
	{
		if (lErr = m_pMemCol->GetData(hPosW, &pPw))
		...
		cuda_bce_with_logits_loss_bwd_kernel_pw << <CAFFE_GET_BLOCKS(n), CAFFE_CUDA_NUM_THREADS >> > (n, nN, nC, x, y, pw, btm_diff);
	}
	else if (hW != 0)
	{
		if (lErr = m_pMemCol->GetData(hW, &pW))
		...
		cuda_bce_with_logits_loss_bwd_kernel_w << <CAFFE_GET_BLOCKS(n), CAFFE_CUDA_NUM_THREADS >> > (n, nN, nC, x, y, w, btm_diff);
	}
	else
	{
	...
	if (bMeanReduction)
	{
		...
		return scal(n, (T)(1.0 / nN), btm_diff);
	}
...
__global__ void adam_update_kernel(int n, T* g, T* m, T* v, T beta1, T beta2, T eps_hat, T corrected_local_rate)
{
	...
	{
		T fGi = g[i];
		T fMi = m[i] * beta1 + fGi * (1 - beta1);
		T fVi = v[i] * beta2 + fGi * fGi * (1 - beta2);
...
template <class T>
long Math<T>::adam_update(int n, long hNetParamDiff, long hValM, long hValV, T fBeta1, T fBeta2, T fEpsHat, T fLearningRate, T fCorrection)
{
	LONG lErr;
	MemoryItem* pNetParamDiff;
	MemoryItem* pValM;
...
template long Math<double>::adam_update(int nCount, long hNetParamDiff, long hValM, long hValV, double dfBeta1, double dfBeta2, double dfEpsHat, double dfLearningRate, double dfCorrection);
template long Math<float>::adam_update(int nCount, long hNetParamDiff, long hValM, long hValV, float fBeta1, float fBeta2, float fEpsHat, float fLearningRate, float fCorrection);
...
template <class T>
long Math<T>::adamw_update(int n, long hNetParamDiff, long hValM, long hValV, T fBeta1, T fBeta2, T fEpsHat, T fLearningRate, T fDecayRate, long hNetParamData, int nStep)
{
	LONG lErr;
	MemoryItem* pNetParamDiff;
...
template long Math<double>::adamw_update(int nCount, long hNetParamDiff, long hValM, long hValV, double dfBeta1, double dfBeta2, double dfEpsHat, double dfLearningRate, double dfDecayRate, long hNetParamData, int nStep);
template long Math<float>::adamw_update(int nCount, long hNetParamDiff, long hValM, long hValV, float fBeta1, float fBeta2, float fEpsHat, float fLearningRate, float fDecayRate, long hNetParamData, int nStep);
...
long Math<double>::mtx_aggregate_rows_sum(int nWidth, int nHeight, double* a, double* ones, double* y)
{
	...
	if (lErr = cublasDgemv(cublas, CUBLAS_OP_T, nCols, nRows, &dfAlpha, a, nCols, ones, 1, &dfBeta, y, 1))
		return lErr | ERROR_CUBLAS_OFFSET;
...
long Math<float>::mtx_aggregate_rows_sum(int nWidth, int nHeight, float* a, float* ones, float* y)
{
	...
	if (lErr = cublasSgemv(cublas, CUBLAS_OP_T, nCols, nRows, &fAlpha, a, nCols, ones, 1, &fBeta, y, 1))
		return lErr | ERROR_CUBLAS_OFFSET;
...
long Math<double>::mtx_transpose(int nWidth, int nHeight, long hA, long hY)
{
	...
	if (lErr = cublasDgeam(cublas, CUBLAS_OP_T, CUBLAS_OP_N, nM, nN, &dfAlpha, a, nN, &dfBeta, a, nM, y, nM))
		return lErr | ERROR_CUBLAS_OFFSET;
...
long Math<float>::mtx_transpose(int nWidth, int nHeight, long hA, long hY)
{
	...
	if (lErr = cublasSgeam(cublas, CUBLAS_OP_T, CUBLAS_OP_N, nM, nN, &fAlpha, a, nN, &fBeta, a, nM, y, nM))
		return lErr | ERROR_CUBLAS_OFFSET;
...
long Math<double>::mtx_dot(int nM, int nN, int nK, long hA, long hB, long hC)
{
	...
	if (lErr = cublasDgemm(cublas, CUBLAS_OP_N, CUBLAS_OP_N, nK, nM, nN, &dfAlpha, b, ldb, a, lda, &dfBeta, c, ldc))
		return lErr | ERROR_CUBLAS_OFFSET;
...
long Math<float>::mtx_dot(int nM, int nN, int nK, long hA, long hB, long hC)
{
	...
	if (lErr = cublasSgemm(cublas, CUBLAS_OP_N, CUBLAS_OP_N, nK, nM, nN, &fAlpha, b, ldb, a, lda, &fBeta, c, ldc))
		return lErr | ERROR_CUBLAS_OFFSET;
...
long Math<T>::calc_batch_dist(int nDistMethod, T fThreshold, int nItemDim, long hS, long hT, long hW, const int nDim0, const int nDim1, LONGLONG* rgOffsets, T* rgDist)
{
	...
	for (int i = 0; i < nDim0; i++)
	{
		...
		if (lErr = cublasSetStream(cublas, streams[i]))
		{
	...
	if (lErr = cublasSetStream(cublas, NULL))
	{
		lErr |= ERROR_CUBLAS_OFFSET;
		goto cleanup;
	}
