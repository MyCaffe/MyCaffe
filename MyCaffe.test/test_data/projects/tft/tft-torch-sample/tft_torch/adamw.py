...
import clr, System
clr.AddReference("C:\\temp\\projects\\TFT\\tft-torch-sample\\OptimizerLib\\OptimizerLib\\bin\\\Debug\\OptimizerLib.dll")
...
def adamw(params: List[Tensor],
    ...
    if foreach and not torch.jit.is_scripting():
        ...
        func = _single_tensor_adamw_Custom #_single_tensor_adamw_OriginalSimple
...
def direct_step(lr, step_t, param, grad, exp_avg, exp_avg_sq, weight_decay=0, beta1=0.9, beta2 = 0.999, eps=1e-08):
    opt.update_step(lr, weight_decay, beta1, beta2, step_t, eps)
    rgW = Array[Single](param.detach().cpu().numpy().flatten())
    rgG = Array[Single](grad.detach().cpu().numpy().flatten())
    rgM = Array[Single](exp_avg.detach().cpu().numpy().flatten())
