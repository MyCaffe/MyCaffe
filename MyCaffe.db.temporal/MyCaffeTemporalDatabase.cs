...
namespace MyCaffe.db.temporal
{
    ...
    public partial class MyCaffeTemporalDatabase : Component, IXTemporalDatabaseBase
    {
        ...
        public int GetTotalSize(int nDsId, Phase phase, int nHistoricalSteps, int nFutureSteps)
        {
            ...
            ValueStreamDescriptor vsd = sd.TemporalDescriptor.ValueItemDescriptors[0].ValueStreamDescriptors[0];
            if (vsd.ItemCount < nStepsPerBlock)
                m_log.FAIL("Stream: " + sd.Name + " - The number of historical and future steps must be less than the number of steps in the temporal data stream.  The steps per block = " + nStepsPerBlock.ToString() + " and the value stream '" + vsd.Name + "' has " + vsd.ItemCount.ToString() + " items.");

            int nRowBlocks = vsd.ItemCount - nStepsPerBlock;
        ...
        public SimpleDatum[] QueryTemporalItem(int nQueryIdx, int nSrcId, DB_LABEL_SELECTION_METHOD? itemSelectionOverride = null, DB_ITEM_SELECTION_METHOD? valueSelectionOverride = null, bool bEnableDebug = false, string strDebugPath = null)
        {
            ...
            return ts.GetData(nQueryIdx, itemSelection, valueSelection, 1, bEnableDebug, strDebugPath);
        }
