...
class PhysionetLearner(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.training_step_outputs.extend(y_hat)
        self.training_step_targets.extend(y)
        return loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.val_step_outputs.extend(y_hat)
        self.val_step_targets.extend(y)
        return [softmax, y]

    def on_validation_epoch_end(self):
        all_preds = torch.stack(self.val_step_outputs)
        all_labels = torch.stack(self.val_step_targets)
        #all_preds = torch.cat([l[0] for l in self.val_step_outputs])
        #all_labels = torch.cat([l[1] for l in self.val_step_outputs])

        auc = auroc(all_preds, all_labels, task="multiclass", num_classes=2)
