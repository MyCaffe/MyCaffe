...
namespace MyCaffe.app
{
    public partial class FormMain : Form
    {
        ...
        private Dictionary<string, string> getCudaPaths()
        {
            ...
            string strFile = AssemblyDirectory + "\\cuda_12.3\\CudaDnnDll.12.3.dll";
            if (File.Exists(strFile))
                rgPaths.Add("CUDA 12.3", strFile);

            strFile = AssemblyDirectory + "\\cuda_12.2\\CudaDnnDll.12.2.dll";
            if (File.Exists(strFile))
                rgPaths.Add("CUDA 12.2", strFile);

            strFile = AssemblyDirectory + "\\cuda_12.1\\CudaDnnDll.12.1.dll";
            if (File.Exists(strFile))
        ...
        private void testDownloadTestDataTftDataOnlyToolStripMenuItem_Click(object sender, EventArgs e)
       {
        ...
        private void testDataDownloadTFTSharpeToolStripMenuItem_Click(object sender, EventArgs e)
        {
            List<string> rgstrUrl = new List<string>();
            string m_strOutputFolder = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData), "MyCaffe\\test_data\\tft-sharpe");

            rgstrUrl.Add("https://signalpopcdn.blob.core.windows.net/mycaffesupport/tft-sharpe_test.zip");
