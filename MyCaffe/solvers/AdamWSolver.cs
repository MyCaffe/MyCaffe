...
namespace MyCaffe.solvers
{
    ...
    public class AdamWSolver<T> : SGDSolver<T>
    {
        ...
        public AdamWSolver(CudaDnn<T> cuda, Log log, SolverParameter p, CancelEvent evtCancel, AutoResetEvent evtForceSnapshot, AutoResetEvent evtForceTest, IXDatabaseBase db, IXPersist<T> persist, int nSolverCount = 1, int nSolverRank = 0, Net<T> shareNet = null, onGetWorkspace getws = null, onSetWorkspace setws = null)
        ...
        public override void ComputeUpdateValue(int param_id, double dfRate, int nIterationOverride = -1)
        {
            ...
            List<double?> net_params_decay = net.all_params_weight_decay;
