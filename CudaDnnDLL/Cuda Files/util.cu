...
bool GetErrorString(long lErr, char* szErr, long lMaxErr)
{
	...
	switch (lErr)
	{
		...
		case ERROR_CUDA_MISSING_NCCL64DLL:
			_snprintf(szErr, lMaxErr, "CUDA: The 'nccl64' DLL is missing from the executable directory!  For example when using version 133, the file 'nccl64_133.dll' should be in the same directory as the executable. (%ld)", lErr);
			return true;
	}
