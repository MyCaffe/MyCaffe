...
class Memory
{
	...
	public:
		...
		long GetRnnWorkspaceCountEx(long hHandle, long hRnnDesc, long hXDesc, size_t* pnWsCount, size_t* pnResCount);
		...
		long RnnForward(long hHandle, long hRnnDesc, long hXDesc, long hXData, long hHxDesc, long hHxData, long hCxDesc, long hCxData, long hWtDesc, long hWtData, long hYDesc, long hYData, long hHyDesc, long hHyData, long hCyDesc, long hCyData, long hWorkspace, size_t nWsCount, long hReserved, size_t nResCount, bool bTraining);
		...
		long RnnBackwardWeights(long hHandle, long hRnnDesc, long hXDesc, long hXData, long hHxDesc, long hHxData, long hYDesc, long hYData, long hWorkspace, size_t nWsCount, long hWtDesc, long hWtDiff, long hReserved, size_t nResCount);
...
template <class T>
inline long Memory<T>::RopeForward(long hRope, int n, long hYdata, long hXdata, int nFreqOffset)
{
	ropeHandle<T>* pR = (ropeHandle<T>*)m_rope.GetData(hRope);
	if (pR == NULL)
		return ERROR_ROPE_NOT_INITIALIZED;

	return pR->Forward(n, hYdata, hXdata, nFreqOffset);
}
...
template <class T>
inline long Memory<T>::FusedCompAddOp(long hFusedComp, FusedCompOp nOp, DataType dtCompute, T fPadding, long hTensor1, long hTensor2, long hTensor3, long hTensor4, long* plIntermediateTensor)
{
	fusedcompHandle<T>* pFc = (fusedcompHandle<T>*)m_fusedcomp.GetData(hFusedComp);
	if (pFc == NULL)
		return ERROR_FUSEDCOMP_NOT_INITIALIZED;

	return pFc->AddOp(nOp, dtCompute, fPadding, hTensor1, hTensor2, hTensor3, hTensor4, plIntermediateTensor);
}
